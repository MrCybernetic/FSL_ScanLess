/*Corrected to work with ScanLess and use xBridge2 Protocol*/
...
#define MIN_V 3000 // battery empty level
#define MAX_V 5000 // battery full level
#define MAX_BLE_WAIT 80 // Maximum bluetooth re-connect time in seconds 
#define SLEEP_TIME 25 // SleepTime (7 points are about 1 minute)
#define MAX_NFC_READTRIES 12 // Amount of tries for every nfc block-scan
...
SoftwareSerial ble_Serial(6, 5); // RX | TX
...
void setup() {
  ...
  delay(100);
  ble_Serial.write("AT+NAMELimiTTerScan");
  delay(500);
...
void SetProtocol_Command() {
  ...
  {
    if (bdebug) Serial.println("Protocol Set Command OK");
    NFCReady = 1; // NFC is ready
  }
  else
  {
    if (bdebug) Serial.println("Protocol Set Command FAIL");
...
void Inventory_Command() {
  ...
  {
    if (bdebug) Serial.println("Sensor in range ... OK");
    NFCReady = 2;
  }
  else
  {
    if (bdebug) Serial.println("Sensor out of range");
    NFCReady = 1;
  }
}
...
float Read_Memory() {
  ...
  {
    if (bdebug) Serial.print("Read Memory Block Command FAIL");
    NFCReady = 0;
    readError = 0;
  }
  return 0;
...
String Build_Packet(float glucose) {
  ...
  msg.size = sizeof(msg);
  msg.cmd_code = 0x00;
  msg.raw = glucose*1000; // raw_value
  msg.filtered = msg.raw;
  msg.dex_battery = batteryPcnt;  // simulate good dexcom transmitter battery
  msg.my_battery = readVcc();
  msg.dex_src_id = 0xA5B1AE;
  msg.delay = 5000;
  msg.function = 0x01;

  String packet = "";
  packet+=String(sizeof(msg));
  packet+=String(0x00);
  packet+=String(glucose*1000);
  packet+=String(msg.raw);
  packet+=String(batteryPcnt);
  packet+=String(readVcc());
  packet+=String(0xA5B1AE);
  packet+=String(5000);
  packet+=String(0x01);
...
void send_data(unsigned char *msg, unsigned char len)
{
  ...
  delay(40);
  for ( i = 0 ; i < len ; i++ ) {
    if (bdebug) Serial.print(msg[i], HEX);
  }
}
...
void wakeUp() {
  ...
  {
    delay(1000);
    if (bdebug) Serial.print("Waiting for BLE connection ...");
